...
pub fn route(req: RouteRequest<'_>) -> RouteResult {
    ...
    // `pwrflag::emit`. Every flag lands on a coordinate this sheet
    // already draws: a pin of its own net for a sheet-local signal net,
    // and for a global rail the best of that rail's existing `power:*`
    // glyph anchors — which is what `host_bodies` / `sheet_bodies` are
    // for (scoring which anchor has room for the chevron).
