...
const BASELINE: &[(&str, &str, &str, f64, f64, f64, &str)] = &[
    ...
    // are byte-identical.
    //
    // Regenerated wholesale for the ADR-40 promotion of
    // `dc-series-column-pinned` to the default, PLUS the pin-anchoring fix
    // to `dc_column::apply_dc_columns` that the promotion needed.
    //
    // Two reasons, both nameable. (1) The promotion itself: a whole-placer
    // swap, so wholesale movement is expected. (2) The DC-series column is
    // now anchored on each member's SHARED PIN rather than on its origin
    // (CLAUDE.md § "Constraints are pin-anchored"), so every column member
    // whose connecting pin is offset from its origin -- every BJT --
    // shifts by that offset, and the whole column re-seats on the shared
    // pins' barycenter. `common_emitter`'s RC and Q1 now share one
    // collector x, which is what
    // `rail_convention::common_emitter_rc_above_q1_collector_seed`
    // asserts and what the pre-fix tree failed.
    //
    // Sink-measured across the suite for the pin-anchoring fix alone
    // (pre-fix control sink vs post-fix, same tree, same seed):
    // v5 48 -> 43, v16.bends 141 -> 137, v16.branches 31 -> 26,
    // f6 79 -> 77, crossings 10 -> 9, detour 23.42 -> 23.43 (flat),
    // f5 unchanged -- and q5 30 -> 44, q3 28 -> 35, the two verifiers in
    // the suite that read symbol ORIGINS rather than pins or ink. See
    // ADR-40 for why that split is the expected signature of the fix.
    //
    ...
    ("cascode_amp", "#PWR1", "power:GND", 46.99, 64.77, 0.0, ""),
    ("cascode_amp", "#PWR2", "power:GND", 46.99, 81.28, 0.0, ""),
    ("cascode_amp", "#PWR3", "power:GND", 59.69, 74.93, 0.0, ""),
    ("cascode_amp", "#PWR4", "power:GND", 55.88, 81.28, 0.0, ""),
    ("cascode_amp", "#PWR5", "power:+12V", 46.99, 36.83, 0.0, ""),
    ("cascode_amp", "#PWR6", "power:+12V", 59.69, 31.75, 0.0, ""),
    ("cascode_amp", "CB2", "Device:C", 46.99, 77.47, 0.0, "y"),
    ("cascode_amp", "CE", "Device:C", 55.88, 77.47, 0.0, ""),
    ("cascode_amp", "CIN", "Device:C", 35.56, 57.15, 90.0, ""),
    ("cascode_amp", "COUT", "Device:C", 85.09, 46.99, 90.0, ""),
    (
        "cascode_amp",
        "Q1",
        "Device:Q_NPN_BCE",
    ...
    ("cascode_amp", "RB1", "Device:R_US", 46.99, 40.64, 0.0, ""),
    ("cascode_amp", "RB2", "Device:R_US", 46.99, 50.8, 0.0, ""),
    ("cascode_amp", "RB3", "Device:R_US", 46.99, 60.96, 0.0, ""),
    ("cascode_amp", "RC", "Device:R_US", 59.69, 35.56, 0.0, ""),
    ("cascode_amp", "RE", "Device:R_US", 59.69, 71.12, 0.0, ""),
    (
        "common_emitter",
        "#FLG1",
        "power:PWR_FLAG",
    ...
    ("common_emitter", "CE", "Device:C", 60.96, 58.42, 0.0, "y"),
    ("common_emitter", "CIN", "Device:C", 35.56, 46.99, 90.0, ""),
    ("common_emitter", "COUT", "Device:C", 73.66, 43.18, 90.0, ""),
    (
        "common_emitter",
        "Q1",
        "Device:Q_NPN_BCE",
    ...
    ("common_emitter", "R1", "Device:R_US", 40.64, 41.91, 0.0, ""),
    ("common_emitter", "R2", "Device:R_US", 40.64, 52.07, 0.0, ""),
    ("common_emitter", "RC", "Device:R_US", 55.88, 35.56, 0.0, ""),
    ("common_emitter", "RE", "Device:R_US", 55.88, 58.42, 0.0, ""),
    (
        "compensated_divider",
        "#FLG1",
        "power:PWR_FLAG",
    ...
    ("rc_phase_shift", "C1", "Device:C", 39.37, 63.5, 0.0, ""),
    ("rc_phase_shift", "C2", "Device:C", 53.34, 57.15, 0.0, ""),
    ("rc_phase_shift", "C3", "Device:C", 68.58, 57.15, 0.0, ""),
    ("rc_phase_shift", "CE", "Device:C", 92.71, 63.5, 0.0, "y"),
    ("rc_phase_shift", "CIN", "Device:C", 80.01, 48.26, 90.0, ""),
    ("rc_phase_shift", "COUT", "Device:C", 114.3, 44.45, 90.0, ""),
    (
        "rc_phase_shift",
        "Q1",
        "Device:Q_NPN_BCE",
    ...
    ("rc_phase_shift", "RB", "Device:R_US", 83.82, 36.83, 0.0, ""),
    ("rc_phase_shift", "RC", "Device:R_US", 96.52, 35.56, 0.0, ""),
    ("rc_phase_shift", "RE", "Device:R_US", 96.52, 58.42, 0.0, ""),
    (
        "resistor_ladder_ref",
        "#FLG1",
        "power:PWR_FLAG",
    ...
    ("two_stage_amp", "#PWR1", "power:GND", 40.64, 55.88, 0.0, ""),
    (
        "two_stage_amp",
        "#PWR10",
        "power:+12V",
    ...
    ("two_stage_amp", "#PWR2", "power:GND", 54.61, 62.23, 0.0, ""),
    ("two_stage_amp", "#PWR3", "power:GND", 58.42, 72.39, 0.0, ""),
    ("two_stage_amp", "#PWR4", "power:GND", 69.85, 55.88, 0.0, ""),
    ("two_stage_amp", "#PWR5", "power:GND", 85.09, 62.23, 0.0, ""),
    ("two_stage_amp", "#PWR6", "power:GND", 78.74, 72.39, 0.0, ""),
    ("two_stage_amp", "#PWR7", "power:+12V", 40.64, 38.1, 0.0, ""),
    (
        "two_stage_amp",
        "#PWR8",
        "power:+12V",
    ...
    ("two_stage_amp", "#PWR9", "power:+12V", 69.85, 38.1, 0.0, ""),
    ("two_stage_amp", "CC", "Device:C", 64.77, 46.99, 90.0, ""),
    ("two_stage_amp", "CE1", "Device:C", 58.42, 68.58, 0.0, "y"),
    ("two_stage_amp", "CE2", "Device:C", 78.74, 68.58, 0.0, "y"),
    ("two_stage_amp", "CIN", "Device:C", 35.56, 46.99, 90.0, ""),
    ("two_stage_amp", "COUT", "Device:C", 102.87, 43.18, 90.0, ""),
    (
        "two_stage_amp",
        "Q1",
        "Device:Q_NPN_BCE",
    ...
    ("two_stage_amp", "RB1", "Device:R_US", 40.64, 41.91, 0.0, ""),
    ("two_stage_amp", "RB2", "Device:R_US", 40.64, 52.07, 0.0, ""),
    ("two_stage_amp", "RB3", "Device:R_US", 69.85, 41.91, 0.0, ""),
    ("two_stage_amp", "RB4", "Device:R_US", 69.85, 52.07, 0.0, ""),
    ("two_stage_amp", "RC1", "Device:R_US", 54.61, 35.56, 0.0, ""),
    ("two_stage_amp", "RC2", "Device:R_US", 85.09, 35.56, 0.0, ""),
    ("two_stage_amp", "RE1", "Device:R_US", 54.61, 58.42, 0.0, ""),
    ("two_stage_amp", "RE2", "Device:R_US", 85.09, 58.42, 0.0, ""),
    (
        "wien_bridge_osc",
        "#FLG1",
        "power:PWR_FLAG",
